...
import { extractPublicKeyFromWIF } from "../src";
...
export const globalParams: ConfigParams = {
  ...
  bondHolderPublicKey: process.env.BOND_HOLDER_PRIVATE_KEY
    ? extractPublicKeyFromWIF(process.env.BOND_HOLDER_PRIVATE_KEY, network)
    : undefined,
  protocolPrivKey: process.env.PROTOCOL_PRIVATE_KEY,
  protocolPublicKey: process.env.PROTOCOL_PRIVATE_KEY
    ? extractPublicKeyFromWIF(process.env.PROTOCOL_PRIVATE_KEY, network)
    : undefined,
  covenantPrivKeys: process.env.COVENANT_PRIVKEYS?.split(","),
  covenantPublicKeys: process.env.COVENANT_PRIVKEYS
    ? process.env.COVENANT_PRIVKEYS.split(",").map((privKey) =>
        extractPublicKeyFromWIF(privKey, network)
      )
    : undefined,
