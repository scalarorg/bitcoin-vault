...
const prepareTaprootInput = (publicKey: string, network: bitcoin.Network): InputByAddress => { 
  const publicKeyNoCoord = Buffer.from(publicKey, "hex").subarray(1, 33);
  const outputScript = bitcoin.payments.p2tr({ pubkey: publicKeyNoCoord, network }).output;
  return {
...
export const defineAddressType = (address: string, network: bitcoin.Network): AddressType => {
  try {
...
export const prepareExtraInputByAddress = (address: string, publicKey: string, network: bitcoin.Network): InputByAddress => {
  ...
  // For P2WPKH address, we need to get redeemScript
  let redeemScript;
...
export const signPsbt = (network: bitcoin.Network, privkey: string, unsignedPsbt: bitcoin.Psbt, finalize: boolean = true)
...
export const psbtValidator = (pubkey: Uint8Array, msghash: Uint8Array, signature: Uint8Array): boolean => {
